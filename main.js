...
const statsBarContainer = document.querySelector(".stats-bar-container");
const statsBarFront = document.querySelector(".stats-bar-container .front-bar");
const statsBarBack = document.querySelector(".stats-bar-container .back-bar");
...
const statsBarBackgroundSize = getComputedStyle(root).getPropertyValue("--stats-bar-background-size");
// const statsBarBackgroundPositionX = getComputedStyle(root).getPropertyValue("--stats-bar-background-position-x");
...
function updateStats(flipResult) {
  ...
  const percentageChange = totalNumberFront+totalNumberBack === 1 ? 50 : Math.abs(+statsTotalPercentageFront.textContent - totalPercentageFront);

  if(percentageChange > 0) {
    statsTotalPercentageFront.textContent = totalPercentageFront;
    statsTotalPercentageBack.textContent = totalPercentageBack;
  
    statsBarFront.style.width = `${totalPercentageFront}%`;
    statsBarBack.style.width = `${totalPercentageBack}%`;

    let statsBarBackgroundPositionXChangeMultiplier =  percentageChange>=25 ? 10 : Math.min(10, percentageChange);
    if(flipResult!==0) statsBarBackgroundPositionXChangeMultiplier*=-1;
  
    root.style.setProperty("--stats-bar-background-position-x-change-multiplier", statsBarBackgroundPositionXChangeMultiplier);
    statsBarContainer.classList.add("bar-background-position-animation");
    setTimeout(() => {
      statsBarContainer.classList.remove("bar-background-position-animation");
    }, latestFlipsAnimationDuration);
  }
